...
def main() -> None:
    ...
    agent_name = os.environ.get("AZURE_AI_AGENT_NAME", "RedTeamAgentTest")
    ...
    with (
        ...
        eval_run_name = f"Red Team Agent Safety Eval Run for {agent_name} -" + str(
            int(time.time())
        )
        ...
        taxonomy = project_client.evaluation_taxonomies.create(
            name=agent_name, body=eval_taxonomy_input
        )
        ...
        print(
            f"Red teaming Taxonomy created for agent: {agent_name}. Taxonomy written to {taxonomy_path}"
        )
        ...
        while True:
            ...
            if run.status == "completed" or run.status == "failed":
                ...
                output_items_path = os.path.join(
                    data_folder, f"redteam_eval_output_items_{agent_name}.json"
                )
                # Create the data folder if it doesn't exist
                os.makedirs(data_folder, exist_ok=True)
