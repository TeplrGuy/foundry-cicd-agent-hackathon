...
# Use AZURE_AI_PROJECT_ENDPOINT (same as createagent.py)
myEndpoint = os.getenv("AZURE_AI_PROJECT_ENDPOINT") or os.getenv("AZURE_AI_PROJECT")
...
def existingagent():
    ...
    with get_tracer().start_as_current_span(
        "ExistingCICDAgent", kind=SpanKind.CLIENT
    ) as current_span:
        ...
        response = openai_client.responses.create(
            input=[
                {
                    "role": "user",
                    "content": "Summarize the RFP for virginia Railway Express project?",
                }
            ],
        ...
        for output_item in response.output:
            if (
                ...
                and output_item.type == "mcp_approval_request"
            ):
                            ...
                            if (
                                ...
                                for i, annotation in enumerate(
                                    ...
                                    if hasattr(annotation, "file_citation"):
                                        ...
                                        print(
                                            f"      Source: {citation.file_name if hasattr(citation, 'file_name') else 'N/A'}"
                                        )
